...
```{r}
model_keras_base %>% evaluate(x_test, y_test) %>% sqrt()
```
...
```{r}
model_keras_small <- keras_model_sequential() %>% 
  # architecture
...
```{r}
model_keras_big <- keras_model_sequential() %>% 
  # architecture
...
```{r}
hist_small <- model_keras_small %>% fit(x = x_train, y = y_train, validation_split = 0.25, 
                                        epochs = 10, # How often shall we re-run the model on the whole sample
                                        batch_size = 128 # How many observations should be included in every batch
                                        )
```
...
```{r}
hist_big <- model_keras_big %>% fit(x = x_train, y = y_train, validation_split = 0.25, 
                                        epochs = 10, # How often shall we re-run the model on the whole sample
