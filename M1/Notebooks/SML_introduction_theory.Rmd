...
* "The field of Machine Learning seeks to answer the question "How can we build computer systems that *automatically improve with experience*?" - Carnegie Mellon University

## Supervised vs. Unsupervised ML {.smaller}

### Unsupervised ML

Tasks related to pattern recognition and data exploration, in case there yet does not exist a right answer or problem structure. Main application
...
* Concerned with labeling/classification/prediction tasks
* Currently driving >90% ML applications in research, industry, and policy.
...
* Unsupervised ML: Creating formerly undefined output/labels.
...
* The coefficient indicates the slope of this linear function.
...
* How do we do that?
* Technically, we just change the distribution from `gaussian` to `binomial`
...
* It shows their tradeoff over the all probability threshholds for positive classification.
...
Lets see how models with different levels of complexity would interpret the relationship between $x$ and $y$:
...
* Bagged models
   * Models redrawing from different distributions
* Boosted models
...
* Repeat till stop criterium reached, which leads to a tree-like structure.
